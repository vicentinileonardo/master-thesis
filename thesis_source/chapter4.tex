...
A synchronization / mapping mechanism is needed to deal with K8s Custom Resources representing a generic workload and the cloud provider specific K8s Custom Resources used for the provisioning of VM with the respective K8s operators developed by Cloud Providers.
The K8s Operator should look at the generic CR, mutated by policy decisions with the necessary fields, and create the correspondent cloud provider-specific CR.

What the Operator must watch?
Check if schedulingRegion and schedulingTime are set. This would mean that the generic CR has received the scheduling proposed by the AI inference (and other policies).
If those fields are not present it means that the generic CR still has to receive the scheduling information.

The resources managed by this K8s Operator are just the generic, provider-agnostic CRs representing a generic VM. The provider-specific resources are built by this K8s Operator using the generic CR as a blueprint but this Operator will not watch and act on these resources after the creation. (Is this true?)


What to do inside Reconcile()?
continuously check and “wait” if scheduling information have been added to the generic CR (schedulingProvider, schedulingRegion, schedulingTime)
create the provider-specific resource when schedulingTime is “close” (critical step, pitfalls)
set a field in the generic CR that will testify that the corresponding provider-specific resource has been created (therefore assumed to be provisioned?). 


What happens after the provider-specific resource has been created?
Examples: 
what happens if I manually modify the provider-specific CR cpu field from 2 to 4 (inside the cluster)?
what happens if I modify my provisioned instance with the aws console (web UI)?


How to link the 2 Custom Resources?
Which is the best way to actually link a generic CR to a provider specific CR. Something like UUID? Or another approach like ObjectMeta.Name and ObjectMeta.Namespace is better?

How to handle the termination logic of a VM?
Deleting the provider-specific CR inside our K8s container should trigger the correspondent operator that will delete the resources previously provisioned on the cloud provider

Cloud-provider specific fields
This K8s Operator should also contain the logic to deal with cloud-provider specific fields in order to create a valid provider-specific Custom Resource.
In this case we are not referring to different naming conventions for the same concept (Azure vmSize and AWS instanceType) but rather additional fields like Azure resourceGroup.

Candidate solutions for the development of the K8s Operator
kubebuilder 




additional challenge of this custom operator (solved by helm template engine)
Note that we cannot set scheduling time in the AzureVirtualMachine Custom Resource. There is no such dedicated field.
This means that it is our K8s synchronization operator’s duty to create the AzureVirtualMachine CR at the right time. Once the AzureVirtualMachine CR is created, the provisioning (managed by Azure Operator) will start.
If the K8s synchronization operator creates the correspondent provider-specific CR right away, without a waiting logic, the provision on the cloud provider will start in a time different than schedulingTime.
...
\section{Kubernetes Mutating Webhook}
