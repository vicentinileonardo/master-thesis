...
When dealing with multi-cloud workloads with Kubernetes as a control plane, a \textb{synchronization and mapping mechanism} (broker) is required to bridge the gap between:
\begin{itemize}[itemsep=0.2pt, topsep=1pt]
...
As a matter of fact, a CompositionDefinition is pointing to a versioned Helm chart that is stored in a Helm repository.
...
Listing \ref{lst:values.yaml} shows an example of the VmTemplate values.yaml file, which defines the fields of the VM resource.
...
For the purpose of the system, we levareged Helm templating to dynamically generate cloud-provider-specific resources from a generic VM resource.
As a matter of fact, the Helm chart contains all the three cloud provider specific set of templates necessary for a VM provisioning but only one set of templates is used at a time.
...
As previously described in section XYZ, this logic is crucial for the system duo to the fact that cloud provider operators do not support scheduling time metadata in their CRs and therefore as soon as a CR is created the cloud provider operator will provision the resource immediately.
...
The minimum set of resources needed for vm provisioning on Azure through Azure service operator is is:
...
In the context of our system, a Kubernetes Mutating Webhook is used to enforce policies for workload scheduling by modifying K8s custom resources (VmTemplates) with the data from policies. 
...
Open Policy Agent (OPA) is an open-source general-purpose \textbf{policy engine} that enables unified policy enforcement across several types of environments. OPA provides a declarative language called Rego enabling a paradigm known as ``\textbf{Policy as Code}" \cite{opa_docs}.
...
\subsection{OPA integration end-to-end workflow}
...
Other cloud resources that have a higher level of abstraction (e.g., containers, Kuberneteds Pods, serverless functions) expose a different set of metrics.
...
Metrics collected at hypervsisor level are not fine-grained
e.g. an hypervisor could be able to provide the memory allocated to a VM, but not the memory actually used by the VM

some metrcis can only be gathered by specific agents installed on the VMs

\subsubsection{Cloud providers agents}
...
One of the goal of these agents comparared to standard cloud-agnostic agents is to provide a more convenient ..
...
In the context of the system, power consumption metrcis could be used to estimate the carbon footprint of a VM instance.
...
As briefly mentioned in the beginning of this section, there is no adopted standard, there is not something similar to FOCUS yet;
